...
book ss() {
	...
	book head = NULL;
	book p, p2, pr = NULL;
	fp = fopen("bookInformation.dat", "ab+");     //Open files in a read-only manner
	if(fp == NULL) {
...
student tt() {
	...
	fp = fopen("studentInformation.dat", "ab+");     //Open files in a read-only manner
	if(fp == NULL) {
		printf("[Error] Cannot find or open file\n");
	}
	while( !feof(fp) ) {       //Determine whether the file location flag moves to the end of the file 
...
void save_book(book p) {
	FILE *fp;    //The pointer of the file
	fp = fopen("bookInformation.dat", "ab");   //Open a binary file named bookInformation by additional opening file and writing data at the end of the file
	if(fp == NULL)
	{
		printf("[Error] Cannot find or open file");
	}
...
void delete_book() {
	do {
		...
		free(p);
		fp = fopen("bookInformation.dat","wb");
		if(fp == NULL)
			printf("[Error] Cannot find or open file\n");
		...
		if(head != NULL) {
			p = head->next;
			fp = fopen("bookInformation.dat","ab");
			if(fp == NULL)
				printf("[Error] Cannot find or open file\n");
			while(p != NULL)
			{
...
void save_student_information(student p) {
	FILE *fp;    //The pointer of the file
	fp = fopen("studentInformation.dat", "ab");   //Open a binary file named studentInformation by additional opening file and writing data at the end of the file
	if(fp == NULL)
	{
		printf("[Error] Cannot find or open file");
	}
	...
	if( identity == 0 ) {
		...
		while(1) {
			...
			if(c == '1') {
				...
				if(head != NULL) {
					p = head->next;
					fp = fopen("studentInformation.dat","ab");
					if(fp == NULL)
						printf("[Error] Cannot find or open file\n");
					while(p != NULL) {
						if( fwrite(head, sizeof(struct BOOK), 1, fp) != 1 )
