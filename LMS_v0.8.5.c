...
void save_student_information(studnet p) {
	FILE *fp;    //The pointer of the file
...
void register_identity() {
	...
	do {
		c = getch();
		if(c == '1') {
			save_student(newStudent);    //Store book information in files
			system("cls");
			printf("Saving....");
			Sleep(500); 
			printf("|  -----------------------------------------  |\n");
		...
		do {
			...
			if( strlen(inNum) != 10 ) {
				...
				if(strcmp(stu->password, inPassword) == 0) {
					...
					sleep(400);
					identity = 1;
					preStudent->number = stu->number;
					preStudent->password = stu->password;
					preStudent->name = stu->name;
					preStudent->booknum = stu->booknum;
					preStudent->firstBook = stu->firstBook;
					preStudent->secondBook = stu->secondBook;
					preStudent->thirdBook = stu->thirdBook;
					preStudent->next = stu->next;
	...
	if( identity == 0 ) {
		...
		while(p != NULL) {
			if(p != NULL && strcmp(p->number, preStudent->number) == 0) {
				p->number = preStudent->number;
				p->password = preStudent->password;
				p->namepreStudent->name;
				p->booknumpreStudent->booknum;
				p->firstBook = preStudent->firstBook;
				p->secondBook = preStudent->secondBook;
				p->thirdBook = preStudent->thirdBook;
				p->next = preStudent->next;
			}
			p = p->next;
		}
		...
		do {
			...
			if(p && p->exist == 1 ) {
				if( preStudent->firstBook == null || preStudent->secondBook == null || preStudent->thirdBook == null ) {
					p->exist = 2;
					p->stu_number = preStudent->number;
					while(1) {
