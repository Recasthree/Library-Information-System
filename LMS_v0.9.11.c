	...
	if( identity == 0 ) {
		...
		do {
			...
			while(p != NULL && p->x != x)
				p = p->next;
			if(p && p->exist == 1 ) {
				if( strlen(preStudent->firstBook) != 10 || strlen(preStudent->secondBook) != 10 || strlen(preStudent->thirdBook) != 10 ) {
					p->exist = 2;
					strcpy(p->stu_number, preStudent->number);
					while(1) {
						if( strlen(preStudent->firstBook) != 10 ) {
							strcpy(preStudent->firstBook, p->number);
							break;
						}
						if( strlen(preStudent->secondBook) != 10 ) {
							strcpy(preStudent->secondBook, p->number);
							break;
						}
						if( strlen(preStudent->thirdBook) != 10 ) {
							strcpy(preStudent->thirdBook, p->number);
							break;
						}
					}
				}
				else if( strlen(preStudent->firstBook) == 10 && strlen(preStudent->secondBook) == 10 && strlen(preStudent->thirdBook) == 10 ) {
